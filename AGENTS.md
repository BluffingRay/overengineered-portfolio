...
- **⚠️ `package.json` `overrides` are LOAD-BEARING — jsdom stays pinned at 26.1.0, never bump past it** (the Aug-30/31 three-night `ERR_REQUIRE_ESM` war). jsdom ships **external** (Next's default `serverExternalPackages`), so the deployed Lambda `require()`s its whole dep tree at request time with the *function's* Node — and from jsdom 27.4 up that tree contains ESM-only packages (`@exodus/bytes` via whatwg-url, `@csstools/*` 4.x via cssstyle→css-color, `parse5@8`) that crash with `ERR_REQUIRE_ESM`. Three traps learned the hard way: (1) **works-on-localhost proves nothing** — local Node 24 supports `require(esm)`, the Lambda's Node does not; (2) **`transpilePackages` cannot help** — jsdom reads files off disk at runtime (`default-stylesheet.css`), bundling it is structurally impossible; (3) **`engines.node` did NOT move the function runtime** — the Vercel dashboard Settings→Node.js Version dropdown is the real knob. Audit method for any candidate tree: `npm pack` each package and read the tarball `package.json` `exports` map for a `require` condition (a local `require()` test is worthless on Node ≥22.12, and `npm view` hides exports maps). Full saga + method: `docs/specs/esm-require-nightmare-postmortem.md` (local-only, like all specs).
...
**Phase 3.6 — Mobile/responsive optimization (shipped 2026-09-01):**
- **Mobile hero centered-top**: all 4 hero designs route through the `useHeroLayout` hook (hydration-safe `useIsDesktopWidth()`). On mobile, `split` collapses to `centered` (the two-col grid doesn't narrow well); `centered` and `banner` pass through unchanged so banner keeps its full-bleed backdrop on mobile. The centered branch defaults the media to top unless the doc explicitly sets `mediaPosition: 'bottom'`, and `HeroForm` clears `mediaPosition` on layout switch so a stale "bottom" never silently overrides the mobile default. DefaultHero's bottom-media block guarded against double-render.
- **Mobile 2-col grids**: all grid/blog designs + entry-list use `grid-cols-2` on mobile with a mobile gap reduction. EditorialGrid/EditorialBlog intentionally left at 1-col mobile (design-specific judgments).
- **Scrollable tabs** (cross-platform): extracted `useScrollableTabs<T>` hook + `ScrollableTabStrip` component after a mid-stream DRY refactor removed ~50 lines of duplication across `PortfolioView`/`HostedPortfolioView`/`PlaygroundView`. Edge fade gradient on every breakpoint; clickable `‹ ›` floating circular arrows on desktop only. Chrome scrollbar hidden via `pb-2.5 -mb-2.5` + outer `overflow-hidden` (the padding+negative-margin trick — `scrollbar-gutter: stable` was tried and reverted because it brought the scrollbar back). Known quirk: Chrome positions the floating arrows a couple px below the tab text (Firefox is perfect); accepted as a Chrome scrollbar-gutter quirk, larger offsets break Firefox.
- **UtilityBar responsive**: `PortfolioChrome` controls wrapper switched from `flex shrink-0` (forced one-column stack on narrow screens) to `flex w-full flex-wrap ... lg:w-auto lg:shrink-0` gated on `controlsFullWidth = Boolean(adminRow)` — edit mode wraps at <1024px, visitor mode keeps the compact single-row layout. Edit/Done got an accent tint when inactive; Log out got a red tint. Inner groups (accent picker + actions) gained `flex-wrap`.
- **Logout double-confirm fix**: `handleLogout` now checks `isDirty()` BEFORE calling `auth.logout()` and shows a single confirm. A new `setIntentionalNav()` / `isIntentionalNav()` module-level flag in `lib/hostedDoc.ts` suppresses the `useHostedDoc` `beforeunload` listener so the native "Leave site?" dialog doesn't stack on top.
- **Hosted fixed cluster additions**: `useHostedDoc.reload()` (manual always-available fetch+replace, with dirty confirm) + Reload and Log out buttons added to the fixed bottom-right cluster in hosted edit mode (Save / Dashboard / Reload / Log out). Logout in the cluster uses the host's `handleLogout` via a new `onLogout` prop. Product B keeps Logout in the top toolbar (B has a real password login per Phase 4b).
- Full plan + outcome: `docs/specs/mobile-optimization.md`. Verification: `tsc --noEmit` clean, `eslint` clean (one pre-existing unused-import warning in `useHostedDoc.ts:5` not from this work).
- **Two accepted Chrome quirks** (documented in the spec, no further work): (1) the floating tab scroll arrows sit ~2px below the tab text on Chrome (Firefox is perfect); 3px upward nudge is the compromise. (2) When the tab strip's horizontal overflow toggles, elements below get pushed down a few px — a "ghost" of the hidden scrollbar reserving layout space despite being visually hidden. `scrollbar-gutter: stable` / `stable both-edges` make it worse (bring the scrollbar back in both Chrome and Firefox). The only true fix is restructuring to `transform: translateX()`-based scrolling (loses native mobile swipe momentum); cost not worth it. Ship as-is.
