...
- **Mobile hero centered-top**: all 4 hero designs force `effectiveLayout = isDesktop ? layout : 'centered'` via a new hydration-safe `useIsDesktopWidth()` hook. Mobile always falls through to the centered branch (banner becomes desktop-only). DefaultHero's bottom-media block guarded against double-render.
